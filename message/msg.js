...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
        ...
        const fgif = {key: {participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})},message: {"videoMessage": { "title":`© ${setting.ownerName}`, "h": `Hmm`,'seconds': '359996400', 'gifPlayback': 'true', 'caption': `© ${setting.fake}`, jpegThumbnail: fs.readFileSync(setting.pathimg)}}}
		...
		const textImg = (teks) => {
			return conn.sendMessage(from, { text: teks, jpegThumbnail: pp_bot }, { quoted: fgif })
		}
...
case prefix+'blur':
                 ...
                 conn.sendMessage(from, {caption: `*© ${setting.botName}*`, image: {url: `https://hitomi-rest-api.herokuapp.com/api/blur?url=${q}&apikey=${setting.hitokey}`}, buttons: but, footer: `${setting.footer}` }, {quoted: fgif})
                 break
	        // Downloader Menu
			/*case prefix+'tiktok':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
...
case prefix+'listsewa':
       ...
       case prefix+'ceksewa': 
            ...
            case prefix+'kirim':
            if (args.length < 2) return reply(`Kirim perintah ${command} nomer|Teks\nContoh ${command} 628xxxxxxxxx|Woi\n\nAWALI DENGAN 62!`)
            if (!isOwner) return reply(mess.OnlyOwner)
                var text1 = q.split('|')[0] ? q.split('|')[0] : q

                var text2 = q.split('|')[1] ? q.split('|')[1] : ''
                reply(`Pesan Sukses Terkirim`)
                var teks = `*[ PESAN DARI OWNER ]*\nDari : Owner\nUntuk : Kamu\nPesan : *${text2}*`
                var chat = [{buttonId: `${prefix}siap`, buttonText: { displayText: "Siap Bang 👍" }, type: 1 }]
                conn.sendMessage(`${text1}`, { caption: teks, image : fs.readFileSync('./media/surat.jpg'), buttons: chat, footer: `${setting.footer}` }, { quoted: fgif })
                break
case prefix+'siap':
...
break
            case prefix+'quotesmaker':
                 ...
                 reply(mess.wait)
                 var but = [{buttonId: `${prefix}menu`, buttonText: { displayText: "Back To Menu" }, type: 1 }]
                 conn.sendMessage(from, {caption: `*© ${setting.botName}*`, image: {url: `https://leyscoders-api.herokuapp.com/api/quote-maker?text=${q}&apikey=${setting.leyskey}`}, buttons: but, footer: `${setting.footer}` }, {quoted: fgif})
                 break
...
case prefix+'wallml': case prefix+'wallpaperml':
				...
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random Wallpaper", image: { url: data.result.list.gambar }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fgif })
			    limitAdd(sender, limit)
				break
...
reply(mess.wait)
conn.sendMessage(from, {caption: `*© ${setting.botName}*`, image: {url: data.result.result}}, {quoted: fgif})
break
case prefix+'sketch':
if (args.length < 2) return reply(`Kirim perintah ${command} ${setting.ownerName}`)
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
...
if (args.length < 2) return reply(`Kirim perintah ${command} ${setting.ownerName}`)
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
var data = await fetchJson(`https://yuzzu-api.herokuapp.com/api/textpro/pencil?text=${q}`)
reply(mess.wait)
conn.sendMessage(from, {caption: `*© ${setting.botName}*`, image: {url: data.result}}, {quoted: fgif})
break
case prefix+'thunder':
if (args.length < 2) return reply(`Kirim perintah ${command} ${setting.ownerName}`)
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
